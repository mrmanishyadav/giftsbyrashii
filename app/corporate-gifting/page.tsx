  ...
  return (
    <StorefrontFrame>
      <main className="corporate">
        ...
        {logos.length > 0 && <section className="corporate-trust shell"><span>Trusted for employee joy, festive gifting and client milestones</span><div>{logos.map((logo) => <a href={logo.url ?? '#'} key={logo.id} className="client-logo">{logo.logo_url && <span><AdaptiveImage src={logo.logo_url} alt={logo.name} fill sizes="120px" /></span>}<b>{logo.name}</b></a>)}</div></section>}
        ...
        {gallery.length > 0 && <section className="corporate-gallery"><div className="shell"><div className="section-heading"><span className="eyebrow">RECENTLY CURATED</span><h2>Work that sparked joy</h2></div><div>{gallery.map((item) => <article className="managed-campaign" key={item.id}><AdaptiveImage src={item.image_url} alt={item.title} fill sizes="320px" /><span>{item.company || item.campaign || 'GiftsByRashii campaign'}</span><b>{item.title}</b></article>)}</div></div></section>}
        ...
        {faqs.length > 0 && <section className="faq-section shell"><h2>Corporate gifting FAQs</h2>{faqs.map((faq) => <details key={faq.id}><summary>{faq.question}</summary><p>{faq.answer}</p></details>)}</section>}
      </main>
    </StorefrontFrame>
  );
}
